...
```{r}
mdf %>% filter(Course == "BUS 499", Section == "BUS 499 GW1") %>% group_by(Semester) %>% summarise(n = n())
mdf %>% filter(Course == "BUS 499", Section == "BUS 499 GW1", Semester == "Spring 2020") %>% group_by(Assessment.Original, Name) %>% summarize(n = n()) %>% knitr::kable()
```

COnfirmed: PLO2a is clicked for all students.
```{r}
mdf %>% filter(Section == "BUS 499 GW1", Assessment == "Annotated Bibliography", Semester == "Spring 2020", str_detect(Name, "Scholarly"))
```

Three should be all the levels: missing good and basic.
...
```{r}
mdf %>% filter(Course == "BUS 499") %>% group_by(Section) %>% summarise(n = n())
```
