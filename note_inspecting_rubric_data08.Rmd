...
================================================================================
```{r}
# Note the files have been edited.
# Rows with "Summary:" shift some columns and cause chaos...
# If focusing on those rows, however, they can be used conveniently to get scores.
path <- "C:\\Users\\kentr\\OneDrive - California University of Pennsylvania\\Data\\Internal Direct Assessment\\"
x1 <- read_csv(paste0(path, "MBA Comprehensive Exam Summer 2019.csv"))
x2 <- read_csv(paste0(path, "MBA Comprehensive Exam Fall 2019.csv"))
x3 <- read_csv(paste0(path, "MBA Comprehensive Exam Spring 2020.csv"))
xx <- x1 %>% bind_rows(x2) %>% bind_rows(x3)
head(xx)
```

```{r}
comp <- xx %>% filter(grepl("Summary", `Q Title`)) %>%
  transmute(Username, field = `Bonus?`,
            raw_score = `Summary Score`, full_score = `Summary Out Of`,
            score = raw_score / full_score)
comp
comp %>% group_by(field) %>% summarise(mean = mean(score), n = n(), zeros = mean(score == 0), min = min(score))
```
