...
tbl2 <- non %>% transmute(Course, Section, Semester, Assessment,
                          Name = "No Rubric", PLO, Met.UND = np/n,
                          Met.GRD = Met.UND, n, Rubric = FALSE)
...
Finally, aggregate over `Course` over `Section` and `Semester`. Average needs to be weighted by the size of each section.
```{r}
tbl4 <- tbl3 %>%
...
That's wrong. One has 43 students (in 3 sections) and the other has 15 (in 1
section), then n should be 58. But I can't simply add them up to avoid double
count. Aggregation at `tbl4` introduced another inconsistency. Ones from `tbl1`
is in `Section`-`Assessment`-`Name`, while ones from `tbl2` is in `Section` (no
disaggregation of assessment or rubric rows).  Not problematic for `HRM 400` and
`ECO 421` because their observations are entirely replaced. For `BUS 499 GW1`, I
can enter each rubric row.


Another issue is found in MBA. Since I distinguished summative/formative, it
picks up both and make the graph messed up. I can solve this by making yet
another source with only relevant past data... or add a switch for
summative/formative.


- [x] Enter each row of BUS 499
- [x] Deal with varying goals
- [ ] Deal with summative/formative objectives >> Overhaul functions...
