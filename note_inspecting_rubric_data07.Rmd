...
list.rrows <- list(
  "Accounting" = c(rep("No Rubric", 2), NA, "No Rubric"),
...
# MIS, MKT
x1 <- stack(list.plotags)
x2 <- stack(list.rrows)
x3 <- stack(list.goals)

names(x1) <- c("PLO.Tags", "Program")


x4 <- bind_cols(x1, "Rubric.Rows" = x2[, 1]) %>% bind_cols("Goals" = x3[, 1])
write_csv(x4, path = "data/mapping_keys.csv")
```
