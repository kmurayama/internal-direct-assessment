...
- Accounting: All clear except PLO 3, whose course was taught by Rogan Howard (ACC 318 001 Spring). I don’t remember what we agreed to do about this. Retrieve exam data if possible?
- Business Core: All clear. I’m double-checking each for missing data.
- Economics: All clear. Paul will need to make the mapping more explicit, but that’ll be a task for the next round.
- Finance program: Chris told me that he actually didn’t use rubrics for FIN 302. I asked him to assess outcomes from the records and his memory.
...
```{r}
x <- mdf %>% filter(Course == "BUS 499") %>% group_by(Semester, Section) %>%
...
```
```{r}
mdf %>% filter(Course == "BUS 499") %>% group_by(Instructor, Semester, Section) %>% summarise(n_distinct(UserId))
```
...
```{r}
x <- mdf %>% filter(Course == "BUS 345") %>% group_by(Semester, Section) %>%
  summarise(n = n_distinct(UserId))
x
```
...
### MBA
MBA is mostly "robust" to the missing observations because they are non-rubric/exam.
```{r}
tbl4 %>% filter(str_detect(PLO, "MBA")) %>% knitr::kable(dig = 2)
```

Exception is BUS 771.
```{r}
tbl3 %>% filter(Course == "BUS 771")
```

Okay... some are rubric rows and others are not. Aggregation by weighted mean does not work. There is a check though, through group_by(Rubric). How come it went through undetected? Found it. It was overwritten at `tbl4` aggregation. Fix it...Done. Now the graph for MBA shows a glitch.

This "Exam" originally named Final Exam Score - Case Report 3 doesn't look like an exam, though?

Also, GW1 Summer 2019 is counted twice because Final Exam and Final Exam Part 2 are reduced to Exam. Ask Ed for distinctions.


MBA 7. "Chapter Case Study" and "Comprehensive Project". When it's supposed to be exam scores.
```{r}
tbl3 %>% filter(Course == "BUS 799")
tbl3 %>% filter(Course == "Bus 799")
```

```{r}
tbl3 %>% filter(Course == "HRM 711")
```


Delta
--------------------------------------------------------------------------------
How far are we to achieve goals? Comparison with bar graphs is good, but taking the difference from the goal (*delta*) is better.
...
requested <- levels(factor(req2$Section))
