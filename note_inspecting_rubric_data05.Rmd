...
```{r}
get_outcome <- function(plo, rrow){
  if(is.na(plo)){return(NA)}
  tbl3 %>% filter(PLO == plo, str_detect(Name, rrow)) %>% 
    summarize(Met = mean(Met), n = min(n)) %>% mutate(PLO = plo)
}
get_outcome(plo = "BUS 5, BUS 6", rrow = "5[a|b|c|d]")
...
plos <- list("Accounting" = c("ACC 1", "ACC 2", NA, "ACC 4"),
...
```{r}
is.na(plos[["Accounting"]][3])
get_outcome(plos[[1]][1], rrows[[1]][1])
get_outcome(plos[[1]][2], rrows[[1]][2])
get_outcome(plos[[1]][3], rrows[[1]][3])
get_outcome(plos[[1]][4], rrows[[1]][4])
get_outcome(plos[[3]][1], rrows[[3]][1])
...
```{r}
get_outcomes <- function(plos, rrows){
  lres <- list()
  for(i in 1:length(plos)){
    lres[[i]] <- get_outcome(plos[i], rrows[i])
  }
  lres
}
pname <- "Accounting"
get_outcomes(plos[[pname]], rrows[[pname]])
pname <- "Business Core"
get_outcomes(plos[[pname]], rrows[[pname]])
pname <- "Economics"
get_outcomes(plos[[pname]], rrows[[pname]])
